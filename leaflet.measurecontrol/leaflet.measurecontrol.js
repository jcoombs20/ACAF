  ...
  L.Control.MeasureControl = L.Control.extend({
    ...
    toggle: function () {
      if (this.handler.enabled()) {
        ...
        d3.select(".leaflet-control-draw-measure").style({"background-color":"#ff6666", "border-color":"black", "box-shadow":"-2px -2px 3px rgba(0,0,0,0.4) inset"});
      } else {
        this.handler.enable.call(this.handler);
        d3.select(".leaflet-control-draw-measure").style({"background-color":"lightgreen", "border-color":"aqua", "box-shadow":"4px 4px 3px rgba(0,0,0,0.4) inset"});
        d3.select(".leaflet-draw-toolbar").selectAll("a").style("border-color", "#333333");
      }
    },
